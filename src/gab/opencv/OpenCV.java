...
public class OpenCV {
    ...
    */
    ...
    public ArrayList< ArrayList<Integer> > findCircles()
    {
        Mat circles=new Mat(0,0,0); //Don't care, The Mat will be recreated in the HoughCircles function anyway.
        Imgproc.HoughCircles(getCurrentMat(), circles, Imgproc.CV_HOUGH_GRADIENT,2,getCurrentMat().width()/4, 200, 100,0,0);
        ArrayList< ArrayList<Integer> > arr=new ArrayList< ArrayList<Integer> >();
        
        for(int y=0;y<circles.size().area();y++)
        {
              float foo[]={0,0,0};
              circles.get(0,y,foo);
              ArrayList<Integer> circle = new ArrayList<Integer>();
              circle.add((int)foo[0]); //X
              circle.add((int)foo[1]); //Y
              circle.add((int)foo[2]); //Radius
              arr.add(circle);
        }
